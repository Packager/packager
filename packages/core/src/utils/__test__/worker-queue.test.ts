...
describe("When the concurrency limit is 1", () => {
  it("should throw if concurrency is less than 1", () => {
    assert.throws(() => new WorkerQueue({ concurrency: 0 }));
  });

  it("only execute one promise at one time", (done) => {
    const promiseQueue = new WorkerQueue({ concurrency: 1 });
    promiseQueue.add(() => {
  ...
  it("the return value is the result of the promise", async (done) => {
    const promiseQueue = new WorkerQueue({ concurrency: 1 });
    const promiseResult = await promiseQueue.add(() => {
      return new Promise((resolve) => {
        assert.strictEqual(promiseQueue.ongoingCount, 1);
        setTimeout(() => {
          resolve(1);
...
describe("When the concurrency limit is 2", () => {
  it("only execute not more than two promises at one time", (done) => {
    const promiseQueue = new WorkerQueue({ concurrency: 2 });
    promiseQueue.add(() => {
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve(1);
