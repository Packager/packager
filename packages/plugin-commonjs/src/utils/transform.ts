...
import attachScopes, { extractAssignedNames } from "./attach-scopes";
...
export function transformCommonjs(
  ...
  function isRequireIdentifier(node) {
    ...
    if (node.type === "Identifier" && node.name === "require" /* `require` */) {
      ...
      node.type === "MemberExpression" /* `[something].[something]` */
    ) {
      // `module.[something]`
  ...
  function getRequired(node, name?: any) {
  ...
  function shouldUseSimulatedRequire(required) {
    return (
      ...
      (required.source.startsWith(DYNAMIC_REGISTER_PREFIX) ||
        !required.source.endsWith(".json"))
    );
  }
        ...
        if (isReference(node, parent) && !scope.contains(node.name)) {
          if (node.name in uses) {
            if (isRequireIdentifier(node)) {
              if (isStaticRequireStatement(parent)) {
                return;
              }
    ...
    .concat(
      ...
      sources
        ...
        .map(([source]) => {
          ...
          return `import ${importsDefault ? `${name} from ` : ``}'${getProxyId(
            source
          )}';`;
        })
    )
  ...
  if (shouldWrap) {
    ...
    if (
      !hasDefaultExport &&
      (names.length || ((!isEntry || hasImporters) && !isEsModule))
    ) {
      wrapperEnd = `\n\nvar ${moduleName} = {\n${names
