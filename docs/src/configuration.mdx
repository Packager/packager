...
`Bundle Options` are options which you set when you are bundling your files.
...
<Note>
    Sourcemaps are inlined so when you set this options to true, it will greatly
...
<ApiProps type="{ [moduleName: string]: string }" defaultValue="{}" />
