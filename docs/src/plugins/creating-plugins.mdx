...
import { resolve, dirname } from "./path-utils";
import { PluginLoaderResult } from "packager/types";
import { addCssExport } from "./utils";
...
It's important to keep in my the order that we register plugins in. If you have created a plugin that is a "catch all" for all files in a resolver, it makes sense to register that plugin last because since we are catching all files, all other plugin resolves will never be called.
