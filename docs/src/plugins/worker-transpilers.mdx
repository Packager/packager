...
A Worker Transpiler is a dedicated Transpiler class with a WebWorker that is responsible for transpiling code in the worker thread.

There are a few things which need to followed in order to successfully return the transpiled code to the plugin hooks but before we find this out, let's create a simple Worker Transpiler.
...
import { File, PackagerContext } from "packager/types";
import Transpiler from "packager/transpilers/transpiler";
import MyWorker from "web-worker:./my-worker.ts";

class MyTranspiler extends Transpiler {
    constructor(context: PackagerContext) {
        super("my-transpiler", new MyWorker(), context);
    }
...
// MyWorker.ts
import { TRANSPILE_STATUS } from "packager/transpilers/transpiler";
...
Every Worker Transpiler has to extend `Transpiler` Class that Packager comes with because it handles [Transpiler Statuses](#statuses) and loading additional workers if needed.

The Class that we have declared, doesn't really do anything special and it's very likely that all of your Transpiler classes will look like the one above. All you have to know is that the 3 arguments in the `super` keyword are required as well as the `transpile` method.
